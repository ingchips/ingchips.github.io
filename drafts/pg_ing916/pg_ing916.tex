...
\hypertarget{ch-keyscan}{%
\chapter{硬件键盘扫描控制器（KEYSCAN）}\label{ch-keyscan}}
...
\tightlist
\item
  可配置矩阵，最大支持16行\emph{15列或者15行}16列的键盘矩阵。
\item
  每个单独的行或者列可以设置启用或者禁用。
\item
  可配置时钟。
...
\includegraphics{img/KeyScan/keyscan.jpg}
...
\NormalTok{                printf(}\StringTok{"row%u col%u %s}\SpecialCharTok{\textbackslash{}r\textbackslash{}n}\StringTok{"}\NormalTok{,}
...
\NormalTok{            \}}
            ...
            \ControlFlowTok{switch}\NormalTok{ (no_key_pressed_cnt) \{}
            \ControlFlowTok{case} \DecValTok{0}\NormalTok{: }\CommentTok{// have key pressed}
\NormalTok{                no_key_pressed_cnt++;}
\NormalTok{                printf_key_state();}
                \ControlFlowTok{break}\NormalTok{;}
            \ControlFlowTok{case} \DecValTok{1}\NormalTok{: }\CommentTok{// all key released}
\NormalTok{                no_key_pressed_cnt++;}
\NormalTok{                printf_key_state();}
\NormalTok{                key_state_clear();}
                \ControlFlowTok{break}\NormalTok{;}
...
\subsection{效果}\label{ux6548ux679c}}

\includegraphics{img/KeyScan/keyscanlog.JPG}
...
\hypertarget{ux7279ux70b9-5}{%
\subsection{特点}\label{ux7279ux70b9-5}}
...
\hypertarget{ux6b63ux8f6cux548cux53cdux8f6c}{%
\subsection{正转和反转}\label{ux6b63ux8f6cux548cux53cdux8f6c}}

QDEC是通过采集到phase\_a、phase\_b相邻两次的数值变化来判断外设的运动方向。

顺时针采集数据如图所示：

\begin{figure}

{\centering \includegraphics[width=0.7\linewidth]{./img/QDEC/916_QDEC_clockwise} 

}

\caption{QDEC顺时针采集数据}\label{fig:916-qdec-clockwise}
\end{figure}
...
{\centering \includegraphics[width=0.7\linewidth]{./img/QDEC/916_QDEC_anticlockwise} 

}

\caption{QDEC逆时针采集数据}\label{fig:916-qdec-anticlockwise}
\end{figure}
...
\begin{Shaded}
\begin{Highlighting}[]
\DataTypeTok{static} \DataTypeTok{void}\NormalTok{ QDEC_PclkCfg(}\DataTypeTok{void}\NormalTok{)}
\NormalTok{\{}
    \ControlFlowTok{if}\NormalTok{ ((APB_SYSCTRL->QdecCfg >> }\DecValTok{15}\NormalTok{) & }\DecValTok{1}\NormalTok{)}
        \ControlFlowTok{return}\NormalTok{;}
    \DataTypeTok{uint32_t}\NormalTok{ hclk = SYSCTRL_GetHClk();}
    \DataTypeTok{uint32_t}\NormalTok{ slowClk = SYSCTRL_GetSlowClk();}
    \DataTypeTok{uint8_t}\NormalTok{ div = hclk / slowClk;}
    \ControlFlowTok{if}\NormalTok{ (hclk % slowClk)}
\NormalTok{        div++;}
    \ControlFlowTok{if}\NormalTok{ (!(div >> }\DecValTok{4}\NormalTok{))}
\NormalTok{        SYSCTRL_SetPClkDiv(div);}
\NormalTok{\}}
\end{Highlighting}
\end{Shaded}
...
\DataTypeTok{void}\NormalTok{ test(}\DataTypeTok{void}\NormalTok{)}
\NormalTok{\{}
    \CommentTok{// setup qdec}
\NormalTok{    SYSCTRL_ClearClkGate(SYSCTRL_ITEM_APB_PinCtrl |}
\NormalTok{                         SYSCTRL_ITEM_APB_QDEC);}
\NormalTok{    SYSCTRL_ReleaseBlock(SYSCTRL_ITEM_APB_PinCtrl |}
\NormalTok{                         SYSCTRL_ITEM_APB_QDEC);}
\NormalTok{    PINCTRL_SelQDECIn(}\DecValTok{16}\NormalTok{, }\DecValTok{17}\NormalTok{);    }\CommentTok{// set GPIO16=phase_a, GPIO17=phase_b}

\NormalTok{    SYSCTRL_SelectQDECClk(SYSCTRL_CLK_SLOW, }\DecValTok{100}\NormalTok{);}
\NormalTok{    QDEC_PclkCfg();    }\CommentTok{// set pclk not bigger than sclk_slow}
\NormalTok{    QDEC_EnableQdecDiv(QDEC_DIV_1024);}
\NormalTok{    QDEC_QdecCfg(}\DecValTok{50}\NormalTok{, }\DecValTok{1}\NormalTok{);}
\NormalTok{    QDEC_ChannelEnable(}\DecValTok{1}\NormalTok{);}

    \CommentTok{// print qdec data and direction when rotate the mouse wheel manually}
    \DataTypeTok{uint16_t}\NormalTok{ preData = }\DecValTok{0}\NormalTok{;}
    \DataTypeTok{uint16_t}\NormalTok{ data = }\DecValTok{0}\NormalTok{;}
    \DataTypeTok{uint8_t}\NormalTok{ dir;}
    \ControlFlowTok{while}\NormalTok{(}\DecValTok{1}\NormalTok{) \{}
\NormalTok{        data = QDEC_GetData();}
\NormalTok{        dir = QDEC_GetDirection();}
        \ControlFlowTok{if}\NormalTok{ (data != preData) \{}
            \ControlFlowTok{if}\NormalTok{ (dir) \{}
\NormalTok{                printf(}\StringTok{"data: %d, %s}\SpecialCharTok{\textbackslash{}n}\StringTok{"}\NormalTok{, data, }\StringTok{"anticlockwise"}\NormalTok{);}
\NormalTok{            \} }\ControlFlowTok{else}\NormalTok{ \{}
\NormalTok{                printf(}\StringTok{"data: %d, %s}\SpecialCharTok{\textbackslash{}n}\StringTok{"}\NormalTok{, data, }\StringTok{"clockwise"}\NormalTok{);}
\NormalTok{            \}}
\NormalTok{        \}}
\NormalTok{        preData = data;}
\NormalTok{    \}}
\NormalTok{\}}
\end{Highlighting}
\end{Shaded}
